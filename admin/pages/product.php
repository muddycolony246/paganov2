...
if (isset($_POST['add'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        $upload_dir = 'pages/uploads/'; // Directory dove salvare le immagini
        ...
        if (move_uploaded_file($temp_path, $image_path)) {
            ...
            // Gestisci l'errore nell'upload
            $alertMsg = 'Errore durante l\'upload dell\'immagine';
        }
    }

    if ($name != '' && $category_id != '' && $category_id != '0' && $description != '' && $price != '') {

        $id = $mgr->create(new Product(0, $name, $price, $description, $category_id, $image_path));

        if ($id > 0) {
            echo "<script>location.href='" . ROOT_URL . "admin?page=products-list&msg=created';</script>";
            exit;
        } else {
            $alertMsg = 'Errore durante la creazione del prodotto';
        }
    } else {
        $alertMsg = 'Compila tutti i campi obbligatori';
    }
}
...
if (isset($_POST['update'])) {
    ...
    $image_path = $product->image; // Mantieni l'immagine precedente se non viene inviata una nuova immagine

    // Gestione dell'upload dell'immagine solo se ne viene inviata una nuova
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        $upload_dir = 'pages/uploads/'; // Directory dove salvare le immagini
        $filename = $_FILES['image']['name'];
        $temp_path = $_FILES['image']['tmp_name'];
    ...
    if ($id != '' && $id != '0' && $name != '' && $category_id != '' && $category_id != '0' && $description != '' && $price != '') {
        ...
        $numUpdated = $mgr->update(new Product($id, $name, $price, $description, $category_id, $image_path), $id);

        if ($numUpdated > 0) {
            echo "<script>location.href='" . ROOT_URL . "admin?page=products-list&msg=updated';</script>";
            exit;
        } else {
            $alertMsg = 'Errore durante l\'aggiornamento del prodotto';
        }
    } else {
        $alertMsg = 'Compila tutti i campi obbligatori';
    }
...
<form method="post" enctype="multipart/form-data" class="mt-5">
    ...
    <div class="form-group">
        ...
        <select name="category_id" id="category_id" type="text" class="form-control" value="<?php echo esc_html($product->category_id); ?>">
            ...
            <option <?php if ($product->category_id == '1') echo 'selected'; ?> value="1">Tabacchi</option>
            <option <?php if ($product->category_id == '2') echo 'selected'; ?> value="2">Gratta E Vinci</option>
            <option <?php if ($product->category_id == '3') echo 'selected'; ?> value="3">Bar</option>
            <option <?php if ($product->category_id == '4') echo 'selected'; ?> value="4">Alimentari</option>
            <option <?php if ($product->category_id == '5') echo 'selected'; ?> value="5">Detersivi</option>
        </select>
    </div>
    <div class="form-group">
